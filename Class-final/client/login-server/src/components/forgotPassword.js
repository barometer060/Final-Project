...
class ForgotPassword extends Component {
  ...
  componentWillMount() {
    fetch("http://localhost:8000/rest/api/users/forgot/questions")
      .then(allow => {
        return allow.json();
      })
  ...
  onSubmitSecurity() {
    if (this.refs.email.value === "" || this.refs.answer.value === "") {
      ...
      fetch("http://localhost:8000/rest/api/users/forgot/questionsVerify", {
        method: "POST",
        headers: {
        ...
        .then(verify => {
          ...
          if (verify.verifyData) {
            this.setState({ emailId: this.refs.email.value }, () => {
              console.log("Data here");
            });
          }
          this.setState({
            verifyField: verify.verifyData,
            securityAlert: !verify.verifyData
          });
        });
    }
  }
  ...
  handlePassword(e) {
    ...
    if (this.state.passwordMatch) {
      fetch("http://localhost:8000/rest/api/users/forgot/changePassword", {
        method: "post",
        headers: {
          Accept: "application/json",
          "Content-Type": "application/json"
  ...
  render() {
      ...
      showHide = (
        <div className="head">
          ...
          <div className="btn">
            <button
              className="login"
              onClick={this.onSubmitSecurity.bind(this)}
            >
              Submit
            </button>
          </div>
              ...
              <div className="inputs">
                <div className="email1">
                  <input
                    ...
                    pattern="(?=^.{8,}$)((?=.*\d)|(?=.*\W+))(?![.\n])(?=.*[A-Z])(?=.*[a-z]).*$"
                    placeholder="Enter New password"
                    onInput={this.handleCheck.bind(this)}
                  />
                  {this.state.passChecked && (
                    <span id="okspan" className="glyphicon glyphicon-ok " />
                  )}
                  {!this.state.passChecked && (
                    <span
                      id="notokspan"
    ...
    return (
      <div className="fpage">
        {showHide}
        {this.state.securityAlert && <span>Security response incorrect.</span>}

        {this.state.pwdUpdate && (
          <span>
            <h2 id="updatedPass">Password Updated</h2>
            <Link id="link" to={"/"}>
              Click here to login
            </Link>
          </span>
